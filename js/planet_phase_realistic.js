...
var drawPlanetPhase = (function(){
    ...
    function drawDiscs(outer, inner, blurSize){
        var blurredDiameter, blurredOffset, bkgTopOffset, bkgLeftOffset;
        setCss(outer.box, {
            'position': 'absolute',
        ...
        bkgLeftOffset = blurredOffset;
        bkgTopOffset = ((outer.diameter-blurredDiameter)/2);
        console.log(bkgLeftOffset, bkgTopOffset);
        if(bkgLeftOffset <= 0 && bkgTopOffset <= 0){
            bkgLeftOffset = Math.abs(bkgLeftOffset);
            bkgTopOffset = Math.abs(bkgTopOffset);
        }else if(bkgLeftOffset >=0 && bkgTopOffset >=0){
            bkgLeftOffset = -Math.abs(bkgLeftOffset);
            bkgTopOffset = -Math.abs(bkgTopOffset);
        }else{
            bkgLeftOffset = -Math.abs(bkgLeftOffset);
        ...
        setCss(inner.box, {
            'position': 'absolute',
            'background-image': 'url("../frozenleafstudio.github.io/src/craters.png")',
            'background-repeat': 'no-repeat',
            'background-position':'left ' + bkgLeftOffset + 'px ' + 'top ' + bkgTopOffset + 'px',
            ...
            'boxShadow': 'inset 0px 0px ' + blurSize + 'px ' + (blurSize+2) + 'px ' + 'rgba(0, 0, 0, 0.50)',
